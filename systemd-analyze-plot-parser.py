...
def insert_row(row_list, table_name):
    ...
    insert_query = '''INSERT INTO ''' + table_name + '''(service_name, activation_time, start_time, category) VALUES (%s, NULLIF(%s, "NULL"), %s, %s)'''
...
svg_ignore_strings = ["Linux", "Startup", "Activating", "Active", "Deactivating", "Setting", "Generators", "Loading"]
    ...
    for element in svg_list[i].elements():
        if isinstance(element, SVGText):
            ...
            if(first_half[0] not in svg_ignore_strings) and (not is_float(first_half[0][:-1])):
                if len(first_half) == 2:
                    first_half[-1] = time_stamp_validation(first_half[-1][1:-1]) # (3.456s)
                    mini_list.extend(first_half)
                elif len(first_half) == 3: # [cec.service, (1min, 16.3s)]
                    seconds_section = float(time_stamp_validation(first_half[-1][:-1])) #16.3
